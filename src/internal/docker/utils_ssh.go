...
func saveHostKey(machine MachineOptions) func(hostname string, remote net.Addr, key ssh.PublicKey) error {
...
func writeMachineToConfigFile(machine MachineOptions) {
	machineKey := fmt.Sprintf("%s.%s", config.ClientsSSH.ConfigKey, machine.Name())
...
func initKeyPairFile() {
	...
	if fu.FileExists(privateKeyPath) && fu.FileExists(publicKeyPath) {
